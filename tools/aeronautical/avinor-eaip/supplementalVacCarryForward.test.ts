...
describe('approved VAC carry-forward', () => {
  it('preserves the exact independently reviewed chart manifest and provenance', () => {
    ...
    expect(approved.vacCharts).toHaveLength(1);
    expect(carried.vacCharts).toEqual(approved.vacCharts);
    expect(carried.vacCharts[0]).toMatchObject({
    ...
    expect(carried.vacCharts[0]?.sourceReferences.some(({ sourceType }) => sourceType === 'vac-pdf')).toBe(true);
  });
});
